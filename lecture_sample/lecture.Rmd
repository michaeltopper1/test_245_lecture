...
Assuming all of this is done on the main branch. 

Stage changes (get rady for adding them to a version)
```{bash, eval = F}
git add -A
```

Commit changes (commit to adding these changes to your version)
```{bash, eval = F}
git commit -m "this is a commit to take the staged changes and save them as a version"
```

Push the changes (push)
```{bash, eval = F}
git push
```

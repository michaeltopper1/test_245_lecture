...
Branches are one of the most appealing features of Git. As an example, suppose you want to try a new spin on your paper - you want to try a whole new analysis, but don't necessarily want to "commit" (bad pun) to the changes. This is where branches come in. A branch is essentially a clone of your files. You can make changes, create new documents, etc. but you can do this all without changing your master copy. However, if you like the changes you make on your branch, you can push and merge them into the master copy. Don't like your changes? Then just delete the branch and switch back to the main copy. For the purposes of this document, we will call our main copy of our files the "master branch". This is generally the default on Github, and you should either adopt this convention or find a very intuitive substitute. 
...
As stated before, branches are valuable for independent work when you want to try out new ideas or new paths in your analysis. For instance, suppose you want to try out some triple differences in your paper. You can create a new branch titled "analysis_triple_difference", create new documents, new tables, new R scripts etc. If you want these to be saved on Github (you do), stage and commit the changes you made in the branch, but **do not push unless you are sure you want these changes saved into the master branch**. Remember, the branch is effectively a copy of the master with you "branching off" into new directions. All of your work is going to be saved locally and remotely when you commit the changes, but it will not be part of the master branch until you push and "create a pull request" on Github (more on this in a second).
...
1. Now that you like your changes, you can push them to Github. 
2. I don't like the cahnges 

## Branches when working collaboratively

Consider the following workflow which, as you can see, would cause problems:
